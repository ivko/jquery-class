!function($, window) {
    ...
    window.Function.prototype.bind = function(that) {
        var self = this,
    ...
    $.Class = function(params) {
        ...
        if (has.call(params, "Extends")) {
            ...
            } else if ($.isArray(params.Extends)) for(var e in params.Extends) $instance.extend(params.Extends[e].prototype);
        }
        delete $instance.prototype.Extends;
        $.extend($instance.prototype, params);

        // Mutators
        for(var m in $.Class.Mutators) $.Class.Mutators[m].call($instance, has.call(params, m) ? params[m] : ($.Class.MutatorsDefaults.indexOf(m) >= 0));
        return $instance;
    };
